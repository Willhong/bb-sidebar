...
- Manual ordering plus Recent activity, Date created, and Project sort modes, stored in the plugin database so every window and device agrees
...
- Expandable child-thread indicators with running and attention states, with the child menu pinned inside the visible viewport on phones
...
This is a fork of [yusuf8834/bb-sidebar](https://github.com/yusuf8834/bb-sidebar) with two changes carried over from [bb-plugin-t3sidebar](https://github.com/SawyerHood/bb-plugin-t3sidebar):

- The active sort mode is stored on the server and published on the `active-sort` channel, so a second window or device picks it up instead of keeping its own `localStorage` choice. The value a browser stored before this change is handed up once, so upgrading does not reset the list.
- On a compact viewport the child-thread menu is placed against `visualViewport` and the safe-area insets instead of hanging off the trigger, so it cannot run off the left edge or under a notch, keyboard, or home bar.
